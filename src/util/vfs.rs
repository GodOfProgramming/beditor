...
impl<T> Vfs<T> {
	...
	pub fn new_item(
		...
		self.new_node(path, name, VfsNode::Item { value: item })
	}

	pub fn mkdir(&mut self, path: impl Borrow<VfsPath>, name: impl Borrow<Name>) -> Option<&VfsPath> {
		self.new_node(path, name, VfsNode::Dir).ok()
	}

	/// Not very efficient due to lifetimes
	pub fn mkdir_p<N>(&mut self, mut path: impl Iterator<Item = N>) -> Option<VfsPath>
	where
		N: Into<Name>,
	{
		...
		path
			.fold_while(Some(root), |prev, next| {
				if let Some(prev) = prev {
					let next = next.into();
					match self.new_node(&prev, &next, VfsNode::Dir) {
						Ok(new_dir) => return FoldWhile::Continue(Some(new_dir.clone())),
						Err(existing_child) => {
							if let Some(node) = self.read(&existing_child) {
								match node {
									VfsNode::Dir => return FoldWhile::Continue(Some(existing_child)),
									VfsNode::Item { .. } => {
										self.rm(&existing_child);
										if let Ok(new_dir) = self.new_node(&prev, next, VfsNode::Dir) {
											return FoldWhile::Continue(Some(new_dir.clone()));
										}
									}
								}
							}
